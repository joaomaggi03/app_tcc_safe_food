/**
 * components/ExecucaoInspecao.tsx
 * ---------------------------------------------------------------
 * A EXECUÇÃO DE UMA INSPEÇÃO (RF06 + RF09).
 *
 * Isto era o miolo de `app/inspecao.tsx`. Virou componente porque
 * passou a ter DOIS donos:
 *
 *  - a aba HOJE (`app/index.tsx`), que É a inspeção diária — ela fica
 *    aberta o dia todo, então não faz sentido ser um destino para onde
 *    se navega e de onde se volta;
 *  - a rota `/inspecao` (`app/inspecao.tsx`), por onde entram a
 *    auditoria periódica, a semestral e a diária no modo completo.
 *
 * A tela que hospeda decide o que vem ANTES da lista, pelo `prelude`:
 * na aba Hoje é o cartão de prazos vencidos, na rota é nada. O resto —
 * blocos recolhíveis, gravação a cada toque, conclusão — é igual nos
 * dois, e é por isso que mora num lugar só.
 *
...
export function ExecucaoInspecao({
  estabelecimento,
  trilha,
  modo,
  prelude,
  ...
  modo: ModoInspecao;
  /**
   * Conteúdo da tela que hospeda, desenhado ACIMA do cabeçalho da lista.
   *
   * Vai no `ListHeaderComponent` junto com o resto, e não numa View por
   * fora, para rolar junto: uma faixa fixa no topo roubaria altura da
   * lista o tempo todo, e o que ela diz ("a auditoria venceu") não
   * precisa estar visível enquanto se responde a rotina do dia.
   */
  prelude?: ReactNode;
  /** Botão da tela que hospeda, ao lado do "recolher tudo" no header. */
  ...
  useEffect(() => {
    // O slot do header é UM só, e a aba Hoje também quer um botão nele.
    // Por isso os dois são desenhados juntos aqui, em vez de cada um
    // chamar `setOptions` por conta — a segunda chamada apagaria a
    // primeira, e quem some depende de qual efeito rodou por último.
    const temAlgo = botaoNoHeader || headerExtra;
  ...
  }, [navigation, botaoNoHeader, algumAberto, grupos, headerExtra]);
    ...
    if (resposta === 'nao_se_aplica' && !jaMarcado) {
      Alert.alert(
        ...
        `O item ${item.codigo_rdc} deixará de aparecer nas próximas inspeções deste ` +
          'estabelecimento. Você pode voltar a exibi-lo na aba Estabelecimento.',
        [
